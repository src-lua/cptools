...
def test_create_contest_from_url(tmp_path):
    ...
    with patch('commands.new.load_config', return_value={'author': 'TestUser'}), \
         patch('commands.new.ROOT_DIR', root_dir), \
         patch('commands.new.TEMPLATE_PATH', template_path), \
         patch('commands.new.parse_contest_url', return_value=mock_info), \
         patch('commands.new.get_input', side_effect=['1234', 'A~B']), \
         patch('commands.new.detect_judge') as mock_detect, \
         patch('commands.update.generate_info_md') as mock_gen_md:
